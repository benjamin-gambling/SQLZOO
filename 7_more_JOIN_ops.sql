...
FROM casting JOIN movie m ON movieid = m.id
WHERE yr = 1978
GROUP BY title
ORDER BY COUNT(movieid) DESC, title;
-- 15. List all the people who have worked with 'Art Garfunkel'.
SELECT name
FROM actor a JOIN casting c ON a.id = actorid JOIN movie m ON movieid = m.id
WHERE movieid IN (SELECT m.id
    FROM movie m JOIN casting c ON movieid = m.id JOIN actor a ON a.id = actorid
